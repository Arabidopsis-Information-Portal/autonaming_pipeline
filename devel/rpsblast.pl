...
my $run_rpsblast = "$FindBin::Bin/run_rpsblast_etc.pl";
my $rps_annotation = "$FindBin::Bin/rps_annotation_processing.pl";
...
foreach my $file (@files2) {
	...
	foreach my $partition_file (@blast_partitions) {
		...
		my $blast_cmd = "$run_rpsblast $file $partition_file $dir/$db_name.results.xml"; 
		if ($config) {
			$blast_cmd .= " $config";
		}
		if ($service) {
	...
	unless ($skip_blast) {
		if ($max_job_array > 0) {
			...
			die "No database partitions found in $rpsdb.\n"
		}
	}
...
my $anno_cmd = 	"$rps_annotation --infile $dir_file --results_path $results_path --project_code $grid_code"; 
if ($config) {
...
print "Done processing rpsblast annotations.\n";
