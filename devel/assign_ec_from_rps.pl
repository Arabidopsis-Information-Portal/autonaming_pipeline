...
<--rules_file,-r>
    XML rules file for priam to EC assignments.  An example of this file is located here:
     '/usr/local/db/priam/05-2011/annotation_rules.xml'
...
my $ec_rules = &parse_priam_ec_xml( $rules_file );
...
# %ec_rules defined globally.

sub priam_to_ec {
    my ($top) = @_;
    
    if ($$ec_rules{ $top->{'priam_id'}  } ) {
        return $$ec_rules{ $top->{'priam_id'} };
    }   
...
sub parse_priam_ec_xml {
    my ($inputfile) = @_;
    my %ec;
    my $xml = new XML::Simple(KeyAttr=>'ECs');
    my $data = $xml->XMLin($inputfile); 
    

     foreach my $key (%{$data->{'RULE'}}) {
        if (ref($key) ne "HASH") {
            if (defined($data->{'RULE'}{$key}{'REPRESENTATIVE_PROFILES'}{'PROFILE'}{'ac'})) {
                $ec{$data->{'RULE'}{$key}{'REPRESENTATIVE_PROFILES'}{'PROFILE'}{'ac'}}=$key;
              } 
        }     
     }  
     return \%ec;
...
sub print_bsml {
    ...
    ## if there are multiple EC numbers, then add them all as attributres.
...
sub check_options {
    ...
    my @required_opts = qw( input_file output rules_file );
