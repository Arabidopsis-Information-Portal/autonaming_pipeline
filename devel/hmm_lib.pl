...
sub parse_hmm_hits {
    ...
    if ( $inverse ne '' ) {
        $tb1_cutoff = ( $tb1_cutoff ? $tb1_cutoff : 2000);
        $tb2_cutoff = ( $tb2_cutoff ? $tb2_cutoff : 2000);
        $te1_cutoff = ( $te1_cutoff ? $te1_cutoff : -100);
        $te2_cutoff = ( $te2_cutoff ? $te2_cutoff : -100);
    } else { 
        $inverse    = 0; 
        $tb1_cutoff = ( $tb1_cutoff ? $tb1_cutoff : -2000);
        $tb2_cutoff = ( $tb2_cutoff ? $tb2_cutoff : -2000);
    ...
    my $n = 0;
    while ( $n < @line ) {
		next if ( $line[$n] =~ /^ *RF [ Xx]+$/ );
        # get program used (hmmsearch or hmmpfam or hmmscan).
		if ( $line[$n] =~ /^.?.?(hmm\w+)/ ) {
            $method = $1;
            if ( ($method eq 'hmmscan') || ($method eq 'hmmsearch') || ($method eq 'hmmpfam') ) {
                print "Method: $method\n" if ($debug);
            } elsif ( $method eq '' ) {
                if ( \*FHTAB ) {
                    close FHTAB;
        ...
        if ( $line[$n] =~ /HMMER (\d)\.(\S+) \(/ ) {
            ...
            my ( $maj_ver, $min_ver ) = ( $1, $2 );
            print "Version: $maj_ver.$min_ver\n" if ($debug);
            $version = 2;    # Logical Depth HMMer currently uses 2.2g output format
        } elsif ( $line[$n] =~ /^CLC\w*\s+(\d+)\.(\S+)/ ){
            ## New code for CLC hmmer handling 2009-04-08 by aklump
            my ( $maj_ver, $min_ver ) = ( $1, $2 );
            print "Version: $maj_ver.$min_ver\t" if ($debug);
        ...
        # get HMM db for hmmscan
        if ( $line[$n] =~ /^# target HMM database:\s+.*\/(\S+)/) {
        ...
        if ( $version == 1 ) {						# Does anything still use version 1?
            ...
            # get query sequence name for hmmscan
            if ( $line[$n] =~ /^Query:\s+(\S+)/ ) {
                my $tmp_obj = $1;
                &print_parse_hmm_hit(
                        $db_proc,      $db,             $output,
...
# 2007-05-08 RAR altered regex to accept either N or #D as number of domains column header
            ...
            /^(Model\s+)Description\s+Score\s+E-value\s+(?:N|\#D)/ )
        {
            ...
            while ( $line[ $m + $n + 2 ] =~ /^\S+/ ) {
                ...
                print "$m --- $seq{$m}{header}\t$seq{$m}{comment}\t$seq{$m}{score}".
                        "\t$seq{$m}{e_value}\t$seq{$m}{number}\n" if ($debug);
                ++$m;
            }
            $seq{number} = $m;
        }

        # Get the header, total score, e-value, # of domains for each sequence (hmmscan)
        if ( $line[$n] =~ /^Scores for complete sequence/ ) {
            ...
            while ( $line[ $n + 4 + $m ] !~ /^$/ ) {
                ...
                my @data = split /\s+/, $line[ $n + 4 + $m], 11;
                @{$seq{$m}}{'header','number','e_value','score','comment'} = (@data)[9,8,1,2,10];
        ...
        if ( $line[$n] =~ /^Sequence\s+Domain\s+seq-f\s+seq-t\s+hmm-f\s+hmm-t\s+score\s+E-value/i
            or $line[$n] =~ /^Model\s+Domain\s+seq-f\s+seq-t\s+hmm-f\s+hmm-t\s+score\s+E-value/i) {
        ...
        # get seq-f, seq-t, hmm-f, hmm-t, score, e_value for each domain for hmmscan
        if ( ( $line[$n] =~ /Domain annotation for each model/) &&
             ( $line[$n + 1] =~ /^$/ ) ) {
        ...
        if ( $line[$n] =~ />> (\S+)/ ) {
            ...
            while ( $line[$n + 3 + $m] !~ /^$/ ) {
                ...
                my @data = split /\s+/, $line[$n + 3 + $m] , 17;
                for my $k ( 0 .. $seq{number} - 1 ) {

                    if ($seq{$k}{header} eq $header) {

                        @{$seq{$k}{$data[1]}}{'seq_f','seq_t','hmm_f','hmm_t','score','e_value'} =
                                (@data)[13,14,7,8,3,6];
                        print "$m --- ",join("\t",@data), "\n" if ($debug);
...
sub print_parse_hmm_hit {
    ...
    if ( $db_proc ne '' ) {
        if ( $method eq 'hmmsearch' ) {
            my $ref = &do_query( $db_proc, "SELECT hmm_len, trusted_cutoff, noise_cutoff, hmm_com_name 
                                              FROM $db..hmm2 WHERE hmm_acc = \"$query_obj\" AND is_current = 1"
            );
            ...
            $hmm_com_name =~ s/\t/ /g;
            print "hmm_len, trusted, noise, hmm_com_name: $hmm_len, $trusted_cutoff, $noise_cutoff, $hmm_com_name\n" if ($debug);
...
#            }
            ...
            for my $n ( 0 .. $$seq_r{number} - 1 ) {
                ...
                my $query = "SELECT hmm_len, trusted_cutoff, noise_cutoff, hmm_com_name
                               FROM $db..hmm2 WHERE hmm_acc = \"$$seq_r{$n}{header}\" AND is_current = 1";
            ...
            for my $n ( 0 .. $$seq_r{number} - 1 ) {
                ...
                #my $query = "SELECT hmm_len, trusted_cutoff, noise_cutoff, hmm_com_name FROM hmm3 WHERE hmm_acc = \"$$seq_r{$n}{header}\" AND is_current = 1";
                #my $ref = &do_query( $db_proc, $query );
                $$seq_r{$n}{hmm_len}        = $$ref{hmm_len};
                $$seq_r{$n}{noise_cutoff}   = $$ref{noise_cutoff};
                $$seq_r{$n}{trusted_cutoff} = $$ref{trusted_cutoff};
                $$seq_r{$n}{comment}        = $$ref{hmm_com_name};
                $$seq_r{$n}{comment} =~ s/\t/ /g;
                print
                  "HI seq \#, hmm_len, trusted, noise, hmm_com_name: $n, $$seq_r{$n}{trusted_cutoff}, $$seq_r{$n}{noise_cutoff}, $$seq_r{$n}{comment}\n"
                if ($debug);                
...
# first try hmm2 table
	my $info =
		$db_proc->selectrow_hashref(
			"select hmm_acc, hmm_len, trusted_cutoff, noise_cutoff, gathering_cutoff, hmm_com_name as hmm_com_name, "
