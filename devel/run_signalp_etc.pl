...
my $signalp = "/usr/local/bin/signalp";
...
$in_name =~ s/.fasta//;
my $outfile = "$results_path/signalp_results.$in_name.gff";

my $signalp_cmd = "$signalp -t euk -n $outfile -v $file ";
print "$signalp_cmd\n";
my $program = &get_file_name($signalp);
