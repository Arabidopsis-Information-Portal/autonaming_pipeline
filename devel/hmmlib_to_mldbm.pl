...
while ( my $line = <$ifh> ) {
    ...
    if ( $line =~ m|^//| && defined $att ) {
        ...
        if ( $$att{TC} =~ /([0-9\.\-]+)\s+([0-9\.\-]+)/ ) {
            ...
            warn "failed to parse trusted global and trusted domains for $$att{ACC}, assigning 1000.00\n";
	    $trusted_global=$trusted_domain=1000.00;
        }
        ...
        if ( $$att{NC} =~ /([0-9\.\-]+)\s+([0-9\.\-]+)/ ) {
            ...
            warn "failed to parse noise global and noise domains for $$att{ACC}, assigning 1000.00\n";
	    $noise_global=$noise_domain=1000.00;
        }
        ...
        if ( $$att{GA} =~ /([0-9\.\-]+)\s+([0-9\.\-]+)/ ) {
            ...
            warn "failed to parse gathering global and gathering domains for $$att{ACC}\n";
	    $gathering_global=$gathering_domain=1000.00;
        }
	 ...
	 if ( $$att{ACC} =~ /^(PF[0-9]+)\.[]0-9]+$/ ) { $$att{ACC} = $1 }
 	 print "$$att{ACC}\n";
        $info{$$att{ACC}} = {
                                name => $$att{NAME},
