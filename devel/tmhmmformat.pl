...
# correct gnuplot path
# set filename base to always have wwwdir pre-appended (ie wwwdir option sets output path)
# gnuplot now outputs png instead of eps

# Flush immediately
$| = 1;

#wnelson $gnuplot = "/usr/local/bin/gnuplot-3.7 ";
...
sub print_stuff {
	...
	# Make a plot
	if ( $opt_plot && $plpread ) {
	    make_plot($fn,$id,$len,\@pred);
	    # Make the eps plot
	    system("$gnuplot $fn.gnuplot");
	    if ($opt_html) {
		# Make gif file
		system("$epsmove $fn.eps | $ps2gif > $fn.gif");
#		$file = $opt_wwwdir$fn;
		$file = $fn;
...
set xrange [1:$len]
# Make the ps plot
#wnelson set term postscript eps color solid \"Helvetica\" 30
set term png font \"Helvetica\" 30
#wnelson set output \"$filename.eps\"
